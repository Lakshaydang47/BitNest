...
import { 
  ...
  Check, AlertTriangle, ShieldAlert 
} from 'lucide-react';
...
export default function CreateRepo() {
  ...
  const handleCreate = async (e) => {
    ...
    // 2. Fetch the current logged-in user's ID
    // IMPORTANT: Replace this with however you store your user's ID (Context, Redux, JWT)
    ...
    try {
      ...
      setError(err.response?.data?.error || "Failed to create repository. Please try again.");
    } finally {
      setLoading(false);
    }
  };
                ...
                <label className={`flex items-start gap-4 p-4 rounded-xl border-2 cursor-pointer transition-all ${isPublic ? 'border-indigo-500 bg-indigo-50/30' : 'border-slate-200 hover:border-slate-300 hover:bg-slate-50'}`}>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">Anyone on the internet can see this repository. You choose who can commit.</p>
                    </div>
                  </div>
                </label>
                ...
                <label className={`flex items-start gap-4 p-4 rounded-xl border-2 cursor-pointer transition-all ${!isPublic ? 'border-indigo-500 bg-indigo-50/30' : 'border-slate-200 hover:border-slate-300 hover:bg-slate-50'}`}>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">You choose who can see and commit to this repository.</p>
                    </div>
                  </div>
                </label>
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              ...
              >
                {loading ? (
                  ...
                  <>
                    <Check size={16} /> Create Repository
                  </>
                )}
              </button>
            </div>
